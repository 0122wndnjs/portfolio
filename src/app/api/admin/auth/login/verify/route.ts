...
export async function POST(request: Request) {
  try {
    ...
    const expectedChallenge = consumeChallenge(body.challengeId, "login");
    if (!expectedChallenge)
      return NextResponse.json(
        { error: "로그인 요청이 만료됐습니다. 다시 시도하세요." },
        { status: 400 },
      );
    const saved = credentials().find((item) => item.id === body.response.id);
    if (!saved)
      return NextResponse.json(
    ...
    await createSession();
    audit("passkey.login", { credentialId: saved.id });
