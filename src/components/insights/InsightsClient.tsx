...
export default function InsightsClient({ posts }: { posts: Post[] }) {
  ...
  return (
    <main className="min-h-screen" style={{ background: "#f4f5fb" }}>
      ...
      {/* Masthead */}
      <div className="relative overflow-hidden pt-32 pb-20" style={{ borderBottom: "1px solid rgba(14,13,31,0.06)" }}>
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
          <span
            className="font-black tracking-tighter"
            style={{ fontSize: "clamp(8rem, 22vw, 20rem)", color: "rgba(14,13,31,0.02)", whiteSpace: "nowrap" }}
          >
            INSIGHTS
          </span>
        </div>
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-8 lg:px-12">
          ...
          >
            <div>
              ...
              <h1 className="font-black tracking-tight leading-[0.9]" style={{ fontSize: "clamp(3.5rem, 9vw, 8rem)", color: "#0e0d1f" }}>
                What I've Written.
              </h1>
            </div>
            <div className="hidden md:flex flex-col items-end gap-1 pb-2">
              <span className="text-[10px] font-mono tracking-widest uppercase" style={{ color: "rgba(14,13,31,0.2)" }}>Total</span>
              <span className="font-black text-4xl" style={{ color: "rgba(14,13,31,0.12)" }}>
              ...
              {categories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      color: isActive ? "#5b4dff" : "rgba(14,13,31,0.3)",
                      border: `1px solid ${isActive ? "rgba(91,77,255,0.4)" : "rgba(14,13,31,0.08)"}`,
                      background: isActive ? "rgba(91,77,255,0.06)" : "transparent",
                    }}
                  >
                    {cat}
                    {isActive && (
      ...
      <div className="mx-auto max-w-6xl px-8 lg:px-12">
        <AnimatePresence mode="wait">
          ...
          >
            {/* Featured */}
            {featured && (
              <div className="py-16" style={{ borderBottom: "1px solid rgba(14,13,31,0.06)" }}>
                <Link href={`/insights/${featured.slug}`} className="group block">
                  <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-start">
                    <div className="shrink-0">
                      <span className="font-black leading-none select-none" style={{ fontSize: "clamp(5rem, 10vw, 9rem)", color: "rgba(14,13,31,0.06)", lineHeight: 1 }}>
                        01
                      </span>
                    </div>
                    <div>
                      <div className="flex items-center gap-4 mb-5">
                        <span
                          ...
                          style={{ color: "rgba(91,77,255,0.8)", background: "rgba(91,77,255,0.08)", border: "1px solid rgba(91,77,255,0.15)" }}
                        >
                          {featured.topic || "Featured"}
                        </span>
                        <span className="text-xs font-mono" style={{ color: "rgba(14,13,31,0.2)" }}>
                          {featured.publishedAt || featured.updatedAt}
                        </span>
                      </div>
                      <h2
                        className="font-bold tracking-tight leading-[1.1] mb-5"
                        style={{ fontSize: "clamp(1.8rem, 3.5vw, 3rem)", color: "rgba(14,13,31,0.9)" }}
                      >
                        {featured.title}
                      </h2>
                      <p className="font-light leading-relaxed mb-7" style={{ fontSize: "1.05rem", color: "rgba(14,13,31,0.38)", maxWidth: "55ch" }}>
                        {featured.summary}
                      </p>
                      <div className="flex items-center gap-3" style={{ color: "#5b4dff" }}>
                    ...
                    >
                      <span
                        ...
                        style={{
                          ...
                          color: isHovered ? "rgba(91,77,255,0.6)" : "rgba(14,13,31,0.07)",
                          lineHeight: 1,
                          minWidth: "3rem",
                        }}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          ...
                          style={{ fontSize: "clamp(1rem, 1.5vw, 1.2rem)", color: isHovered ? "#0e0d1f" : "rgba(14,13,31,0.7)" }}
                        >
                          {post.title}
                        </p>
                        <div className="flex items-center gap-4 text-xs font-mono" style={{ color: "rgba(14,13,31,0.2)" }}>
            ...
            {filtered.length === 0 && (
              <div className="py-32 text-center">
                <p className="text-sm font-mono" style={{ color: "rgba(14,13,31,0.2)" }}>No posts in this category.</p>
              </div>
            )}
          </motion.div>
        </AnimatePresence>
