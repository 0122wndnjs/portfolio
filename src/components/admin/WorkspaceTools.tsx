...
type Report = { start: string; end: string; tasks: ReportItem[]; meetings: ReportItem[]; projects: { id: string; name: string; kind: string; next_action: string; waiting_reason: string }[] };
...
export default function WorkspaceTools() {
  ...
  const reportText = report ? [
    ...
    "", "현재 다음 행동·대기 사항",
    ...report.projects.filter((item) => matches(item.kind) && (item.next_action || item.waiting_reason)).map((item) => `- ${item.name}: ${item.next_action || "다음 행동 미등록"}${item.waiting_reason ? " / 대기: " + item.waiting_reason : ""}`),
  ].join("\n") : "";
  ...
  return <div className="space-y-4">
    ...
    {tab === "search" && <>
      <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); void run(async () => { setResults(await api<Result[]>(`/api/admin/search?q=${encodeURIComponent(query)}`)); setSearched(true); }); }}>
        <input className={fieldClass + " min-w-0 flex-1"} aria-label="통합 검색어" required maxLength={200} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="프로젝트, 작업, 미팅, 메모 검색" />
    ...
    {tab === "export" && <section className="space-y-4 rounded-xl border border-black/5 bg-white p-5"><h2 className="font-semibold">업무 데이터 내보내기</h2><p className="text-sm text-gray-500">프로젝트·작업·미팅·견적·입금·휴지통을 JSON 파일로 저장합니다. 인증 정보와 텔레그램 비밀키는 포함하지 않습니다.</p><a download className={buttonClass + " inline-block"} href="/api/admin/export">JSON 다운로드</a><p className="text-xs text-gray-500">보관용 데이터 파일입니다. 자동 가져오기 기능은 제공하지 않습니다.</p></section>}
  </div>;
}
