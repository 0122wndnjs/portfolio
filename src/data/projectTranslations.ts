...
export const projectTranslationsEn: Record<number, ProjectTranslation> = {
  ...
  26: {
    role: "Planning and full-stack development (monorepo) of a global medical beauty platform. Designed the orchestration-based dev process, integrated 7 social login providers, wired up GA4 event tracking, and handled PWA service worker issues.",
    desc: "A multi-tenant platform covering overseas medical/beauty procedure bookings, payments, and settlement end-to-end — user web, hospital admin, super admin console, and API server all run from a single monorepo. Launched (v1 live).",
    tasks: [
      "Designed a Turborepo + pnpm workspace monorepo architecture with 4 apps (customer, hospital admin, super admin, backend).",
      "Built a multilingual (next-intl) user web with Next.js 15 + React 19, added PWA support (Serwist), and deployed to Cloudflare Workers via OpenNext.",
      "Built the backend API with NestJS 11 + Prisma 6 + PostgreSQL; implemented JWT auth and 7 social login providers (Google/Kakao/Naver/Facebook/Line/Apple/WeChat) with custom OAuth2 flows including JWKS verification.",
      "Integrated Stripe payments and designed automated deposit and settlement processing.",
      "Implemented rewards/coupons, real-time chat, notifications (web push + logs), and photo-attached reviews.",
      "Implemented hospital operations: doctor/staff management and QR-based booking check-in.",
      "Implemented admin domain: audit logs, banner/category/notice/FAQ CMS, and approval workflows.",
      "Wired up GA4 event tracking (sign_up/login/begin_checkout/purchase) and designed an orchestration-based dev process (plan → task spec → implementation → review, kept as separate stages).",
    ],
    features: [
      "Auth/users: JWT auth, 7 social login providers, consent log history, profiles.",
      "Booking/products: hospital and procedure listings, variants, reservations with status history tracking.",
      "Payments/settlement: Stripe payments, deposits, automated settlement (Settlement/SettlementItem).",
      "Rewards/coupons: reward logs, coupon issuance/redemption, welcome coupon seeding script.",
      "Communication: real-time chat (Conversation/Message), push + log notifications, photo reviews with replies.",
      "Hospital operations: hospital/doctor/staff management, hospital images, QR-based booking check-in.",
      "Admin: audit logs, banner/category/notice/FAQ CMS, approval workflows.",
      "Search/favorites: product and hospital search, favorites.",
      "Scheduler: batch job heartbeat monitoring.",
    ],
    impact: [
      "Shipped a global medical beauty platform connecting overseas patients with Korean clinics through its v1 launch.",
      "Built a multi-tenant architecture running 4 apps (user web, hospital admin, super admin, API) from a single monorepo.",
      "Implemented core domain logic including 7 custom OAuth2 social login providers, Stripe payments, and automated settlement.",
      "Applied an orchestration-based development process (plan → task spec → implementation → review) on a real production project.",
    ],
  },
  27: {
  ...
  38: {
    role: "Planning and full-stack development of a golf community platform, unified across web and mobile (iOS/Android)",
    desc: "A golf community web and mobile app offering round tracking, golf course info sharing, and point-economy gamification. Launched (v1 live).",
    tasks: [
      "Built web, iOS, and Android from a single codebase using Next.js 15 (App Router) + Capacitor.",
      "Implemented Supabase (PostgreSQL) with Row-Level Security for data access control and Naver OAuth social login.",
      "Built round/score tracking, golf course discovery with filtering and reviews, round reports, and community rankings.",
      "Designed a point-economy system: credit accrual/spend, daily login bonuses, and a 12-month-inactivity expiration policy.",
      "Implemented a raffle (entry/draw) system with cryptographic verification and a point-shop purchase flow.",
      "Built a notification system combining native push (FCM for Android, APNs for iOS) with PWA web push.",
      "Built the admin system: user/round/report stats dashboard, banner/notice/item/raffle management, and report handling.",
      "Prevented N+1 queries with batched requests fetching multiple resources per API call, and applied WASM-based face-blurring for images.",
      "Set up 14 Playwright E2E tests, 7 Node.js native integration tests, and a GitHub Actions CI/CD pipeline.",
    ],
    features: [
      "Round tracking: add/edit/delete rounds, score (par/stroke) logging, round analysis and stats.",
      "Course discovery: listing/filtering, detail info (location, difficulty, reviews), favorites, live report/review stats.",
      "Community: round reports (photos, comments), course reviews (ratings, comments), likes/comments, national/regional rankings.",
      "Point economy: credit accrual/spend, daily login bonus, report-writing rewards, 12-month-inactivity expiration policy.",
      "Raffles: recurring raffles, point-based entries, cryptographically verified draws, win history.",
      "Point shop: point-based purchases, category filtering, purchase history.",
      "Push notifications: web push (PWA), native push (FCM/APNs), win/event alerts.",
      "Admin: stats dashboard, banner/notice management, item/raffle management, report handling.",
      "Profile: nickname/avatar management, activity history, rankings, my entries.",
      "PWA: home-screen install, offline fallback, app manifest, iOS add-to-home-screen guidance.",
    ],
    impact: [
      "Shipped a cross-platform golf community platform — web + iOS + Android from one codebase — through its v1 launch.",
      "Completed a point-economy system with monthly-active-user tracking and a cryptographically verified raffle system.",
      "Secured reliability with 100+ E2E and integration tests (Playwright/Node.js) and a 200+ item release QA checklist.",
      "Built security across the full stack — RLS, OAuth, role-based permissions, and input validation across database and API layers.",
    ],
  },
  39: {
    role: "Planning and full-stack development of two internal partner web apps for an interior contractor network: an automated contract generator and an expert-certification / project-showcase intake service",
    desc: "Two internal partner tools for interior contractors — a wizard-based standard-contract generator with PDF preview, and an expert-certification and project-showcase submission/review service.",
    tasks: [
      "[Contract generator] Built a 4-6 step wizard flow (basic info → project info → contract terms → warranty → special terms → preview) with Vue 3 (Composition API) + TSX + Zod schema validation.",
      "[Contract generator] Ported the company's real contract and warranty spreadsheet structure (including merged checkbox cells) directly into the data model, so a checkbox click reflects straight into the document.",
      "[Contract generator] Handled real-world contract terms — installment payments, VAT-separate schedules, warranty ratios — as conditional fields, with html-to-image powering live A4-page-by-page preview and thumbnail capture.",
      "[Contract generator] Migrated a production service from React to Vue 3 in its entirety — routing, form state, and document-rendering logic included.",
      "[Certification intake] Built a 3-step registration form (business info, representative/contact info, project showcase) with React Hook Form + Zod and per-step partial validation.",
      "[Certification intake] Built a serverless pipeline in Netlify Functions that batches Google Drive uploads (auto-separated folders and normalized filenames for ID, license, and project-showcase files) with Notion DB registration.",
      "[Certification intake] Designed service-account-based server auth so API keys are never exposed to the client, and built a mobile-first responsive UI (capped at a 480px mobile view on desktop).",
    ],
    features: [
      "Contract generator: 4-6 step wizard, faithful paper-form reproduction, conditional fields for installments/VAT/warranty ratio, live preview with PDF download, a hub landing page for multiple partner tools.",
      "Certification & project-showcase intake: 3-step registration (business / representative-contact / project showcase), real-time validation, image/PDF/ZIP upload with preview, final review before submit, auto-save to Google Drive and auto-registration to Notion DB.",
    ],
    impact: [
      "Automated a contract workflow that used to be copied by hand into spreadsheets by faithfully reproducing the paper contract and warranty forms in code.",
      "Migrated a live production service from React to Vue 3, gaining experience with framework-agnostic form and rendering architecture.",
      "Built a serverless automation pipeline via Google Drive/Notion API integration covering the full flow from certification document intake to ops-team review.",
    ],
  },
};
